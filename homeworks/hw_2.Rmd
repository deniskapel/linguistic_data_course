...
1. How many participants are mentioned in this dataframe? 30

```{r}
mono_socio %>% select(ParticipantID) %>% distinct() %>% count()
```

2. How many of them are males and females? 19 females and 11 males
...
3. Which education levels are mentioned in the dataframe? "College", "UniversityDegree", "Postgraduate", "PhDongoing" 
...
Compare you overview with that reported in Table 1 of the article. Sometimes replication data provided by authors does not allow one to reproduce their results. Gender and handedness correspond to Table 2 of the article. It is difficult to understand how levels of education are grouped because there are only 3 College students (Table 2 indicates 6 secondary students)
...
Can we say that RT approximately follows normal distribution? Which features of RT distribution contradicts this assumption? (E.g. long left tail, long right tail, outliers, skewness, etc.) We can say that it is normally distributed, skewed to the right with a long right tail.
...
Can we say that RTlog approximately follows normal distribution? What features of RTlog distribution contradicts this assumption? (E.g. long left tail, long right tail, outliers, skewness, etc.) It is normally and symetrically distributed. The left tail is longer than the right one. 

### 1.5
Give a summary of `RTlog` distribution (min, max, mean, median, standard deviation)

```{r}
psych::describe(mono$RTlog)
```
...
mono1 <- mono %>%
  filter(RT >= 600 & 
           (RTlog < mean(RTlog) + 3*sd(RTlog) & 
              RTlog > mean(RTlog) - 3*sd(RTlog)
            ) & (
              substr(TypeOfStimuli, 1, 6) != "Filler"))

mono1 <- mono1 %>% 
  select(ParticipantID, TypeOfStimuli, WordOrder, TypeOfQuestion, ResponseAcceptabilityJudgement, RTlog) %>% 
  mutate(ParticipantID = as.factor(ParticipantID),
         ...
         ResponseAcceptabilityJudgement= as.factor(ResponseAcceptabilityJudgement),
         RTlog = RTlog)
...
labels <- mono1 %>% select(TypeOfStimuli) %>% distinct() 

mono1 %>% 
  ggplot(aes(x = interaction(TypeOfStimuli, WordOrder),
             y = ..count..,
             fill = ResponseAcceptabilityJudgement,
             color='black')) +
  geom_bar(position = 'dodge',colour = "black")+
  annotate("text", x = c(2,5), y = 450, label = c("Congruented", "Incongruented"))+
  annotate('text', x=1:6 ,y=-25, label=rep(c('Shape.Color', 'Size-Nationality', 'Comment-Material'), 2))+
  labs(y="", x="", fill="AJ")+
  ...
  theme(plot.margin = unit(c(1, 1, 4, 1), "lines"),
        ...
        axis.line.y = element_line(size = 0.3, linetype = "solid", lineend = 'round'))
...
```{r}
mono1 %>% 
  ggplot(aes(x = WordOrder,
             y = ..count..,
             fill = ResponseAcceptabilityJudgement,
             color='black')) +
  geom_bar(position = 'dodge',colour = "black")+
  labs(y="", x="Order", fill="AJ")+
  scale_fill_grey(start=0.3, end=0.7)+
  theme(plot.margin = unit(c(1, 1, 4, 1), "lines"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
```{r}
mono1 %>% 
  ggplot(aes(x = ResponseAcceptabilityJudgement,
             y = RTlog, fill=WordOrder)) +
  geom_violin(width=0.5) +
  geom_boxplot(width=0.5,alpha=0.2)+
  scale_fill_grey(start=0.3, end=0.7)+
  labs(x="AJ", y='log RT', fill="Order")
```
...
mono1 %>% 
  select(WordOrder, ResponseAcceptabilityJudgement, RTlog) %>% 
  group_by(WordOrder, ResponseAcceptabilityJudgement) %>% 
  summarise(avg = mean(RTlog), plus_stdev = avg+sd(RTlog), minus_stdev = avg-sd(RTlog)) %>% 
  ggplot(aes(x=ResponseAcceptabilityJudgement, group=WordOrder, color=WordOrder)) + 
  geom_line(aes(y=avg))+
  geom_line(aes(y=plus_stdev))+
  geom_line(aes(y=minus_stdev))
```
...
mean_rtlog_long <- mono1 %>% 
  ...
  summarise(rt_log = round(mean(RTlog),2))
...
```
Would you reject null hypothesis (under 5% significance level) according to this test? No, I cannot, p-value > 0.05.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test? If we consider WordOrder values as two independent groups, the population mean of Incongruent is not greater.
...
Whould you reject null hypothesis? Yes, I would, p.value < 0.05

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now? RTlog and WordOrder are not independent, and the mean for Incogruent is greater.

How can you interpret difference with the result of 2.3? Each participant has either congruent or incogruent samples, and we cannot consider them as independant.
...
incong_rtlog <- mono1 %>% 
  ...
  summarise(RTlog=round(mean(RTlog),2), .groups='drop')
...
Use appropriate statistical test to answer the following question: are there any statistically significant difference in logarithm of reaction time for different conditions (types of stimuli)? Choose the test and provide justification for your choice. Provide your code, results and interpretation. 

What is your answer to the question? Since we have more than two groups, I choose to use One-way Anova test instead of applying t.test to each pair of groups. This allows me to test this hypothesese:

- H0 - means of all groups are equal.

- H1 - not all of the groups' means are equal.
...
Based on this test, it is possible to reject a null hypothesis and say that no all groups's means are equal.
...
```
Based on anova test in 3.2, it is possible to say that not all the means of TypeOfStimuli groups are equal. This comes from a tests' p-value =. In 3.3, TukeyHSD was applied for the post-hoc analysis to understand which groups are different in their mean. Based on it, it is possible to say that there is a difference between SubjectiveComment-Material-Shape-Color (p-value = 0.015 < 0.05). There is no difference between Size-Nationality-Shape-Color (p-value = 0.87 > 0.05) and SubjectiveComment-Material-Size-Nationality (p-value = 0.057 > 0.05) subgroups.
```
...
```{r}
lm_model1 <- lm(RTlog ~ ResponseAcceptabilityJudgement, data = mono1)
lm_model2 <- lm(RTlog ~ ResponseAcceptabilityJudgement + TypeOfStimuli, data = mono1)
...
The second model is of a better quality. It adds additional meaningful (p-value < 0.05) parameters to it and multiple R-squared is higher, i.e. we explain more of the dispersion (0.115 < 0.126) 

#### 4.2
Add interaction of two predictors in the model.

```{r}
lm_model3 <- lm(RTlog ~ ResponseAcceptabilityJudgement*TypeOfStimuli, data = mono1)
...
```
Model 3 does not provide any additional features that can be used to predict RTlog. The interaction between predictors is not necessary, and it is possible to keep using Model 2 as less complex.
```
...
```{r}
glm_model1 <- glm(WordOrder ~ ResponseAcceptabilityJudgement, mono1, family = binomial())
glm_model2 <- glm(WordOrder ~ ResponseAcceptabilityJudgement + TypeOfStimuli:ResponseAcceptabilityJudgement, mono1, family = binomial())

anova(glm_model1, glm_model2)
...
```
Model 2's AIC coefficient is higher though it is more complex, and there are a lot of unnecessary predsictors. Model 1 is an option to go.
```
